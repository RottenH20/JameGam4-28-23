...
public class LootDropEditor : Editor
{
    ...
    void ValidateGuaranteedList(LootDrop loot)
    {
        ...
        for (int i = 0; i < loot.GuaranteedLootTable.Length; i++)
        {
            if (loot.GuaranteedLootTable[i].Drop == null) { _prefabError = true; }
            if (loot.GuaranteedLootTable[i].MinCountItem <= 0) { _countError = true; }
            if (loot.GuaranteedLootTable[i].MinCountItem > loot.GuaranteedLootTable[i].MaxCountItem) { _countError = true; }
            if (loot.GuaranteedLootTable[i].Weight < 0) { _weightError = true; }
        }
